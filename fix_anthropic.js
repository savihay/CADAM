...
code = code.replace(
  /\/\/ Convert Anthropic-style message to OpenAI format[\s\S]*?interface OpenRouterRequest \{[\s\S]*?\}/g,
  '',
);
...
fs.writeFileSync('supabase/functions/parametric-chat/index.ts.tmp', code);
console.log('File rewritten safely to .tmp');
